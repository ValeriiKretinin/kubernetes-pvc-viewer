  ...
  return (
    <Menu as="div" className="relative inline-block text-left">
      {({ open }) => {
        ...
        return (
          <>
            <Menu.Button ref={btnRef} className="px-2 py-1 border rounded">Actions</Menu.Button>
            <Transition as={Fragment} enter="transition ease-out duration-100" enterFrom="transform opacity-0 scale-95" enterTo="transform opacity-100 scale-100" leave="transition ease-in duration-75" leaveFrom="transform opacity-100 scale-100" leaveTo="transform opacity-0 scale-95">
              <Portal>
                <Menu.Items style={{ position: 'fixed', top: pos.top, left: pos.left, width: menuWidth }} className="z-[9999] origin-top-right rounded-md bg-white dark:bg-gray-800 shadow-2xl ring-1 ring-black/10 dark:ring-white/10 focus:outline-none">
